...
- **description**: `FastAPIセキュリティチェック実行`
- **prompt**: 以下の詳細な指示を渡す

**エージェントへの指示内容**:

```
FastAPIプロジェクトのセキュリティレビューを実行し、OWASP Top 10に基づいてレポートを生成してください。
...
### 3. 認証・認可の問題（🟡 中リスク）
- 保護が必要なエンドポイントにget_current_user依存性がない
- パスワードハッシュ化の欠如
- JWT検証の欠如
- 所有者チェックの欠如（更新・削除時）
...
以下の形式でマークダウンレポート（security-report.md）を生成：
...
エージェントが完了したら、レポートのサマリー（高リスク件数、中リスク件数、低リスク件数）をユーザーに表示し、`security-report.md` の場所を案内します。
